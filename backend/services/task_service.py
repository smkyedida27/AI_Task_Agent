...
def handle_ai_command(text, user):
    ...
    if intent == "new_task":
        task = Task.objects.create(
            task_texts=task_reference,
            user=user
        )

        return {
            "message": f"Task created chesa poyi task chey: {task.task_texts}",
            "task": task.task_texts
        }

    # 6) no tasks safety
    if not task_list:
        return {"message": "EM lev alantivi"}

    # 7) semantic task match
    matched_task = find_similarity(task_reference, task_list)

    # 8) completed flow
    if intent == "completed_task":
        task = Task.objects.filter(
            task_texts=matched_task,
            user=user
        ).first()
        ...
        if task:
            ...
            return {
                "message": f"Task complete Chesav BAANE: {matched_task}",
                "task": matched_task
            }
    ...
    if intent == "remove_task":
        ...
        if task:
            ...
            return {
                "message": f"Task delete Chesan Po: {matched_task}",
                "task": matched_task
            }

#     return {"message": "Could not understand task action"}
# from tasks.models import Task, CommandHistory
# from tasks.llm_parser import parse_content
# from ai.vector_search import find_similarity
